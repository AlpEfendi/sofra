...
const ctx=vm.createContext({});vm.runInContext(fs.readFileSync('data.js','utf8')+';globalThis.test={dinners,mealsFor};',ctx);
assert.equal(ctx.test.dinners.length,14);
for(let d=0;d<14;d++)for(const p of ['a','b']){const m=ctx.test.mealsFor(d,p);assert.equal(m.length,p==='a'?2:3);assert.equal(m.at(-1).time,'19:00');assert.equal(new Set(m.map(x=>x.id)).size,m.length);for(const x of m){assert.ok(x.lines.length);assert.ok(fs.existsSync('art/'+x.art+'.svg'));assert.ok(x.lines.every(l=>!String(l).includes('undefined')))}}
